...
def _digital_to_pa_scale() -> float:
    ...
    return cal_rms_pa / cal_rms_digital
...
def _extract_loudness_posix(input_dir: Path, output_dir: Path,
                             num_workers: int = 4, chunk_size: int = 10) -> None:
    ...
    # Compute calibration scale once
    ...
    print(f"[mosqito] Processing {len(audio_files)} files in {input_dir} ...")
    for audio_file in tqdm(audio_files):
        out_npy = output_dir / (audio_file.stem + ".npy")
        if out_npy.exists():
            continue
        try:
            signal_pa, fs = _load_as_pa(str(audio_file), scale)
            N, _N_spec, _bark, _time = loudness_zwtv(signal_pa, fs)
            # Save as (T, 1) to match Windows exe shape
            np.save(str(out_npy), np.asarray(N, dtype=np.float32)[:, None])
        except Exception as e:
