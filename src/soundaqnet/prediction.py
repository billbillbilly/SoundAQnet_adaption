...
def load_aq_outputs(paq_dir: str | Path) -> pd.DataFrame:
    ...
    for p in tqdm(files, total=len(files)):
        ...
        (pleasant, eventful, chaotic, vibrant,
         uneventful, calm, annoying, monotonous) = df.iloc[1, 0].split('\t')
...
EVENT_LABELS = [
    'Silence', 'Human_sounds', 'Wind', 'Water', 'Natural_sounds', 'Traffic',
...
def main() -> int:
    ...
    parser.add_argument("--export", default="both", choices=["both", "aq", "event"],
                        help="Which outputs to process (default: both).")
    parser.add_argument("--paq_dir",
    ...
    parser.add_argument("--output_prefix", default="soundAQ",
