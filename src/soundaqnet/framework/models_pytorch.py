...
class SoundAQnet(nn.Module):
    ...
    def forward(self, input, batch_x_loudness):
        ...
        x_k_9_mel = F.relu_(self.k_9_freq_to_1(x_k_9))[:, :, 0][None, :, :]
        ...
        # ── Graph neural network (pure PyTorch, no DGL) ──────────────────────
        # event_embs: [N, B, feat_dim]  where N = max_node_num = 8
        B = event_embs.size(1)
        N = self.max_node_num
        # Rearrange to [B*N, feat_dim] — same node ordering as dgl.batch()
        batch_nodes = event_embs.permute(1, 0, 2).reshape(B * N, -1)
        # Edge features initialised to all-ones during training (data_generator.py).
        # Shape: [B * N², feat_dim]
        feat_dim = batch_nodes.shape[-1]
        batch_edges = torch.ones(
            B * N * N, feat_dim, dtype=batch_nodes.dtype, device=batch_nodes.device
        )
