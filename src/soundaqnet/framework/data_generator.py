...
import time
from soundaqnet.framework.utilities import calculate_scalar, scale, create_folder
...
class DataGenerator_Mel_loudness_graph(object):
    def __init__(self, Dataset_path, node_emb_dim, number_of_nodes=8, seed=42, normalization=True, overwrite=False):
        ...
        if self.normal and (not os.path.exists(normalization_log_mel_file) or
                            not os.path.exists(normalization_loudness_file)):
            try:
